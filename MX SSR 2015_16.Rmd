...
locus_table(diploid_pinf, pop = "Chapingo_2015")
...
gst_mat <- as.dist(gst_mat)


geodist <- read.csv("geo.csv", header = TRUE, check.names = FALSE)

geo_mat <- as.matrix(geodist[,-1])
row.names(geo_mat) <- geodist[, 1]
geodist_mat <- as.dist(geo_mat)

ibd <- mantel.randtest(gst_mat, geodist_mat)
plot(ibd)

library(MASS)
dens <- kde2d(geodist_mat,gst_mat, n=300)
myPal <- colorRampPalette(c("white","blue","gold", "orange", "red"))
plot(geodist_mat, gst_mat, pch=20,cex=1.5)
image(dens, col=transp(myPal(300),.7), add=TRUE)
abline(lm(gst_mat~geodist_mat))
