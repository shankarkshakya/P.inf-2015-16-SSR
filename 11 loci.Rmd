...
for (k in 1:length(plots)){
 temp <- plots[[k]] +  theme(axis.text.x = element_text(face = "bold", size = 12))
 new_plots[[k]] <- temp
...
#new_fst_mat <- new_fst_mat/ (1 - new_fst_mat)
...
plot(geodist_mat, fstdist_mat, xlab = "Geographical distance (km)", ylab = "Genetic distance (FST)", cex = 1.5, cex.lab = 1.2)
abline(lm(fstdist_mat~geodist_mat))
...
diploid_pinf_cc <- clonecorrect(diploid_pinf)
tree_nj <- bruvo.boot(diploid_pinf_cc, replen = repeat_length, 
                   sample = 10, cutoff = 70, tree = "nj")

mycol <- rainbow(6)
mycol[2] <- "#000000"
mycol <- mycol %>% setNames(popNames(diploid_pinf_cc))

x <- apeBoot(tree_nj, tree_nj$node.label)
strata <- dplyr::data_frame(taxa = indNames(diploid_pinf_cc)) %>% cbind(strata(diploid_pinf_cc)) 
...
test_tree <- ggtree(x, layout = "unrooted", branch.length = "none") +
  ggtitle("NJ tree") +
